  ...
  return (
    <div className="flex items-center gap-8 lg:gap-19">
      <div className="md:w-1/2 p-12 tracking-tighter leading-snug">
        <h2 className="mb-7 text-3xl font-bold text-base-300 dark:text-white xl:text-sectiontitle2">{title}</h2>
        <p className="mb-5 text-lg pr-24">{desc1}</p>
        <p className="w-11/12 text-lg pr-24 pb-12">{desc2}</p>
        <Link href={link}>
          {" "}
          {/* Use Link component */}
          <a className="px-8 py-4 text-lg font-medium text-center text-base-300 bg-secondary dark:bg-base-100 dark:text-white rounded-btn">
            {button}
          </a>
        </Link>
      </div>
